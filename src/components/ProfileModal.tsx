...
export const ProfileModal: React.FC<ProfileModalProps> = ({
  ...
  return (
    <section
      className="fixed right-3 top-[4.5rem] z-[70] w-[min(20rem,calc(100vw-1.5rem))] overflow-hidden rounded-xl border border-zinc-800/90 bg-[#111216]/98 shadow-2xl shadow-black/60 backdrop-blur-xl sm:right-4 md:right-8 lg:right-10"
      role="dialog"
      aria-modal="false"
    ...
    >
      ...
      <div className="p-4">
        <div className="flex items-start justify-between gap-3">
          ...
          <button onClick={onClose} aria-label="Close profile" className="rounded-md p-1 text-zinc-600 hover:bg-zinc-800 hover:text-zinc-200">
        ...
        {user ? (
          <div className="mt-4 space-y-3">
            ...
            <button type="button" onClick={() => void onLogout()} className="flex w-full items-center justify-center gap-1.5 rounded-lg border border-zinc-800 px-3 py-2 text-xs font-semibold text-zinc-300 transition hover:bg-zinc-800 hover:text-white">
          ...
          <form onSubmit={(event) => void submitAuth(event)} className="mt-4 space-y-2.5">
            <label className="block text-[10px] font-semibold uppercase tracking-wider text-zinc-500">
              Email
              <input type="email" required autoComplete="email" value={email} onChange={(event) => setEmail(event.target.value)} className={inputClass} placeholder="you@example.com" />
            </label>
            <label className="block text-[10px] font-semibold uppercase tracking-wider text-zinc-500">
              Password
            ...
            <button type="submit" disabled={isSubmitting} className="flex w-full items-center justify-center gap-1.5 rounded-lg bg-violet-600 px-3 py-2 text-xs font-semibold text-white hover:bg-violet-500 disabled:opacity-50">
              {isSubmitting && <Loader2 className="h-3.5 w-3.5 animate-spin" />}
              {mode === 'login' ? 'Sign in' : 'Create profile'}
            </button>
            <button type="button" onClick={() => { setMode((value) => value === 'login' ? 'register' : 'login'); setError(null); }} className="w-full text-[11px] text-zinc-500 hover:text-violet-300">
